...
- `MULTIPLAYER_DESIGN.md` — **design-only** spec for the planned Kahoot-style multiplayer mode (lobby codes, N human seats, turn barrier + optional timer, shared game). Not authoritative and **not yet implemented**; it maps the feature onto the existing engine and names the code to change. Read it before building multiplayer.
