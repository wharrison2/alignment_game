...
class GameLogger:
    ...
    def record(self, state, actions, events):
        self.turns.append({
            ...
            "labs": [lab.snapshot() for lab in state.labs],
            "actions": {lab_id: _action_dict(a) for lab_id, a in actions.items()},
            "events": [{
                "id": e.definition_id, "category": e.category, "klass": e.klass,
                "lab_id": e.lab_id, "model_id": e.model_id,
                "severity": round(e.severity, 3), "impact": round(e.impact, 2),
                "public_text": e.public_text, "true_text": e.true_text,
            } for e in events],
        })
